...
#' @examples
run_simulation<-
  function(simulator, epochs, calculate.pcf=FALSE, pcf_grid, auto_stop_at_plateau = TRUE, plateau_threshold = 8){
  require(dplyr)
  time<-numeric(epochs+1)
  pop<-numeric(epochs+1)
  exp_pop<-rep(list(numeric(epochs+1)), each = 6)
  ...
  alpha = 0.3 # константа экспоненциального сглаживания
  found_plateau = FALSE
  #print("epochs")
  #print(epochs)
